...
A table with no current snapshot still has metadata. Views that require a snapshot
(manifests, entries, files, partitions, and their variants) materialize as **empty
tables with the correct schema** — they never raise a DuckDB "table does not exist"
error. A query like `SELECT COUNT(*) FROM delete_files` always returns 0 when there
are no delete files, not an error.

| Snapshot required? | Metadata tables                                                                                                                                   |
|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| No                 | `snapshots`, `history`, `metadata-log`, `refs`                                                                                                    |
| Yes (may be empty) | `manifests`, `all-manifests`, `entries`, `all-entries`, `files`, `data-files`, `delete-files`, `all-data-files`, `all-delete-files`, `partitions` |
