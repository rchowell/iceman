...
### No `catalog` subcommand
`iceman catalog list` **does not exist**. Use `iceman list` for table/namespace discovery.
To manage catalogs, edit `~/.config/iceman/config.toml` directly.

### No `--type` flag on inspect
```
WRONG:  iceman inspect db.t --type files
RIGHT:  iceman inspect db.t files
```
The metadata table is the **second positional argument**, never a flag.

### Always check the schema before writing SQL
Before writing any `-q SQL` query, run `iceman info <metadata-table>` to get
the **exact column names**. Column names differ from the Iceberg spec and other
engine dialects — do not assume. Examples:
```
iceman info files          # before querying files / data_files / delete_files
iceman info entries        # before querying entries / all_entries
iceman info snapshots      # before querying snapshots
```
...
**Pre-flight**: always run `iceman info <metadata-table>` before writing SQL against
a view. Column names differ from spec docs and other dialects — verify first.
